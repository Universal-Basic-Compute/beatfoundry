...
export async function POST(request: NextRequest, { params }: any) {
  ...
  try {
    ...
    const imageResponse = await fetch(`${process.env.KINOS_API_BASE_URL || 'https://api.kinos-engine.ai/v2'}/blueprints/${process.env.KINOS_BLUEPRINT_ID || 'beatfoundry'}/kins/${foundryId}/images`, {
      method: 'POST',
      headers: {
    ...
    if (!imageResponse.ok) {
      ...
      return NextResponse.json(
        { error: `Failed to generate cover image: ${imageResponse.status}` },
        { status: 500 }
      );
    }
    
    const imageData = await imageResponse.json();
    console.log(`[IMAGES] Image generation response:`, imageData);
    
    // Extract the image URL
    const imageUrl = imageData.data?.url;
    
    if (!imageUrl) {
      console.error(`[IMAGES] No image URL found in response`);
      return NextResponse.json(
